(function() {
    ...
    function renderDecl(decl_index) {
      let display_index = decl_index;
      let content_index = decl_index;
      let category = wasm_exports.categorize_decl(decl_index, 0);

      if (category === CAT_alias) {
        content_index = wasm_exports.get_aliasee();
        category = wasm_exports.categorize_decl(content_index, 0);
      }

      switch (category) {
        case CAT_namespace:
        case CAT_container: {
          renderNav(display_index);
          renderDeclHeading(content_index);
          const members = namespaceMembers(content_index, false).slice();
          const fields = declFields(content_index).slice();
          return renderNamespace(content_index, members, fields);
        }
        ...
        case CAT_type_type: {
          renderNav(display_index);
          renderDeclHeading(content_index);
          const docs_html = declDocsHtmlShort(content_index);
          if (docs_html.length > 0) {
            domTldDocs.innerHTML = docs_html;
            domTldDocs.classList.remove("hidden");
          }
          domSourceText.innerHTML = declSourceHtml(content_index);
        ...
        case CAT_function: {
          renderNav(display_index);
          renderDeclHeading(content_index);
          renderTopLevelDocs(content_index);
          renderParams(content_index);
          renderDocTests(content_index);

          domFnProtoCode.innerHTML = fnProtoHtml(content_index, false);
          domFnProto.classList.remove("hidden");

          const errorSetNode = fnErrorSet(content_index);
          if (errorSetNode != null) {
            const base_decl = wasm_exports.fn_error_set_decl(content_index, errorSetNode);
            renderErrorSet(base_decl, errorSetNodeList(content_index, errorSetNode));
          }
        ...
        case CAT_type_function: {
          ...
          const members = unwrapSlice32(wasm_exports.type_fn_members(content_index, false)).slice();
          const fields = unwrapSlice32(wasm_exports.type_fn_fields(content_index)).slice();
          if (members.length !== 0 || fields.length !== 0) {
            renderNamespace(content_index, members, fields);
          } else {
            domSourceText.innerHTML = declSourceHtml(content_index);
            domSectSource.classList.remove("hidden");
          }
        ...
        case CAT_error_set: {
          renderNav(display_index);
          renderDeclHeading(content_index);
          const errorSetList = declErrorSet(content_index).slice();
          return renderErrorSet(content_index, errorSetList);
        }
        default:
    ...
    function renderNamespace(base_decl, members, fields) {
        ...
        while (true) {
          ...
          switch (member_category) {
            ...
            case CAT_container:
              ...
              continue member_loop;
            case CAT_global_variable:
              varsList.push({original: original, member: member});
              continue member_loop;
            case CAT_function:
              fnsList.push({original: original, member: member});
              continue member_loop;
            case CAT_type:
            case CAT_type_type:
            case CAT_type_function:
      ...
      varsList.sort(byDeclIndexName2);
      ...
      if (typesList.length !== 0) {
          ...
          for (let i = 0; i < typesList.length; i += 1) {
              ...
              aDom.setAttribute('href', navLinkDeclIndex(original_decl));
          }
      ...
      if (fnsList.length !== 0) {
          ...
          for (let i = 0; i < fnsList.length; i += 1) {
              const original_decl = fnsList[i].original;
              ...
              protoCodeDom.innerHTML = fnProtoHtml(decl, false);
              dtDom.onclick = function () { location.hash = navLinkDeclIndex(original_decl); };
      ...
      if (varsList.length !== 0) {
          ...
          for (let i = 0; i < varsList.length; i += 1) {
              const original_decl = varsList[i].original;
              const decl = varsList[i].member;
              ...
              tdNameA.setAttribute('href', navLinkDeclIndex(original_decl));
